...
impl TryFrom<GameStatusDto> for GameStatus {
    ...
    fn try_from(dto: GameStatusDto) -> Result<GameStatus> {
        ...
        Ok(GameStatus {
            ...
            game_players: dto.my_game_participants
                .into_iter()
                .map(GameParticipant::from)
                .collect(),
            my_initial_hand: dto.my_initial_hand
                .into_iter()
                .map(Card::try_from)
                .collect::<Result<_>>()?,
            cards_passed_by_me: dto.cards_passed_by_me
                .into_iter()
                .map(Card::try_from)
                .collect::<Result<_>>()?,
            cards_passed_to_me: dto.cards_passed_to_me
                .into_iter()
                .map(Card::try_from)
                .collect::<Result<_>>()?,
            my_final_hand: dto.my_final_hand
                .into_iter()
                .map(Card::try_from)
                .collect::<Result<_>>()?,
            my_current_hand: dto.my_current_hand
                .into_iter()
                .map(Card::try_from)
                .collect::<Result<_>>()?,
            game_deals: dto.my_game_deals
                .into_iter()
                .map(Deal::try_from)
                .collect::<Result<_>>()?,
...
impl<'a> From<&'a GameStatus> for GameStatusDto {
    fn from(game_status: &'a GameStatus) -> GameStatusDto {
        GameStatusDto {
            ...
            my_game_participants: game_status
                .game_players
                .iter()
                .map(GameParticipantDto::from)
                .collect(),
            my_initial_hand: game_status
                .my_initial_hand
                .iter()
                .map(CardDto::from)
                .collect(),
            cards_passed_by_me: game_status
                .cards_passed_by_me
                .iter()
                .map(CardDto::from)
                .collect(),
            cards_passed_to_me: game_status
                .cards_passed_to_me
                .iter()
                .map(CardDto::from)
                .collect(),
            my_final_hand: game_status
                .my_final_hand
                .iter()
                .map(CardDto::from)
                .collect(),
            my_current_hand: game_status
                .my_current_hand
                .iter()
                .map(CardDto::from)
                .collect(),
